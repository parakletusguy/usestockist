-- Security patch for get_daily_inventory_report RPC
-- Prevents unauthenticated/public invocation of company-wide inventory data
...
BEGIN
  IF auth.uid() IS NULL THEN
    RAISE EXCEPTION 'Authentication required to access inventory reports';
  END IF;
...
-- Revoke execute from public/anon and grant only to authenticated & service_role
REVOKE EXECUTE ON FUNCTION public.get_daily_inventory_report(TIMESTAMP WITH TIME ZONE, TIMESTAMP WITH TIME ZONE, BOOLEAN, TEXT) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_daily_inventory_report(TIMESTAMP WITH TIME ZONE, TIMESTAMP WITH TIME ZONE, BOOLEAN, TEXT) TO authenticated, service_role;
