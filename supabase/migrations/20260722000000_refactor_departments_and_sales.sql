-- Add department column to items and inventory_transactions if not exists
ALTER TABLE public.items ADD COLUMN IF NOT EXISTS department TEXT DEFAULT 'Retail';
ALTER TABLE public.inventory_transactions ADD COLUMN IF NOT EXISTS department TEXT DEFAULT 'Retail';

-- Create reach_sales_reports table for recording Reach daily uploads per retail member
CREATE TABLE IF NOT EXISTS public.reach_sales_reports (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  report_date DATE NOT NULL,
  retail_member_name TEXT NOT NULL,
  file_name TEXT,
  total_items_sold NUMERIC DEFAULT 0,
  total_sales_value NUMERIC DEFAULT 0,
  uploaded_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);

-- Enable RLS on reach_sales_reports
ALTER TABLE public.reach_sales_reports ENABLE ROW LEVEL SECURITY;

-- Create basic RLS policies for reach_sales_reports
CREATE POLICY "Allow authenticated select on reach_sales_reports" 
  ON public.reach_sales_reports FOR SELECT 
  TO authenticated 
  USING (true);

CREATE POLICY "Allow authenticated insert on reach_sales_reports" 
  ON public.reach_sales_reports FOR INSERT 
  TO authenticated 
  WITH CHECK (true);

-- Update get_daily_inventory_report RPC to support department filtering and time ranges
DROP FUNCTION IF EXISTS public.get_daily_inventory_report(TIMESTAMP WITH TIME ZONE, TIMESTAMP WITH TIME ZONE, BOOLEAN);
DROP FUNCTION IF EXISTS public.get_daily_inventory_report(TIMESTAMP WITH TIME ZONE, TIMESTAMP WITH TIME ZONE, BOOLEAN, TEXT);
...
BEGIN
  RETURN QUERY
  WITH item_list AS (
    SELECT id, name, unit_of_measure, category, i.department, unit_cost, low_stock_threshold 
    FROM public.items i
    WHERE (p_department IS NULL OR p_department = 'all' OR i.department = p_department)
  ),
  opening_balances AS (
    SELECT
      t.item_id,
    ...
    WHERE t.transaction_date < p_start_date
      AND (p_department IS NULL OR p_department = 'all' OR t.department = p_department)
    GROUP BY t.item_id
  ),
