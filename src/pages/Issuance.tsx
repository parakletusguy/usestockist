        ...
        <CardContent className="p-3 sm:p-6 pt-0 sm:pt-0">
          ...
          <div className="sm:hidden space-y-2">
            {!filteredLedger?.length ? (
              <p className="text-center text-muted-foreground py-8 text-sm">No issuances recorded yet</p>
            ) : filteredLedger.map(entry => (
              <div key={entry.id} className="rounded-lg border p-3 space-y-1.5">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="text-xs font-bold bg-destructive/10 text-destructive px-2 py-0.5 rounded-full shrink-0">
                    -{entry.quantity} {entry.items?.unit_of_measure}
                  </span>
                </div>
                <div className="flex items-center justify-between text-xs text-muted-foreground">
                  <span>{format(new Date(entry.date), 'PP')} · To: <strong className="text-foreground">{entry.recipient_group}</strong></span>
                  <span>{entry.issued_by}</span>
                </div>
                <div className="flex justify-end pt-1 border-t">
                  <EditDeleteActions onEdit={() => openEdit(entry)} onDelete={() => deleteIssuance.mutate(entry.id)} isDeleting={deleteIssuance.isPending} />
                </div>
              </div>
            ))}
          </div>
